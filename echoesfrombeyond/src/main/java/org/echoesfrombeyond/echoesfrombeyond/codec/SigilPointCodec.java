...
public final class SigilPointCodec implements Codec<SigilPoint>, RawJsonCodec<SigilPoint> {
  ...
  private static SigilPoint decodeFromString(String value) {
  ...
  public SigilPoint decode(BsonValue bsonValue, ExtraInfo extraInfo) {
    return decodeFromString(bsonValue.asString().getValue());
  }

  @Override
  public SigilPoint decodeJson(RawJsonReader reader, ExtraInfo extraInfo) throws IOException {
    return decodeFromString(reader.readString());
  }
  ...
  private static Schema toSchema0(@Nullable SigilPoint def) {
    ...
    if (def != null) schema.setDefault(def.x() + ", " + def.y());
    return schema;
  }
}
